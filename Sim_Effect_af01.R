...
af <- setNames(rep(0.1, 2), mutations)
...
save(dat.fam, file = "SimFamEffect_Orig_01.RData")
...
save(dat.fam, dat.summ, dat.dup.db, dat.dup.db.summ,
     ...
     res.cv, res.cv.orig,
     res.oe, res.auc, res.rbs, res.oe.ss, res.auc.ss, res.rbs.ss,
     file = "Res_Sim_Effect_af01.RData")
...
metrics.mean <- rbind(oe.mean$cv %>% mutate(metric = "O/E", type = "CV on duplicated original data"),
                      oe.mean$val.ext %>% mutate(metric = "O/E", type = "Train on duplicated original data, validate on external data"),
                      oe.mean$train.ext %>% mutate(metric = "O/E", type = "Train on external data, validate on duplicated original data"),
                      auc.mean$cv %>% mutate(metric = "AUC", type = "CV on duplicated original data"),
                      auc.mean$val.ext %>% mutate(metric = "AUC", type = "Train on duplicated original data, validate on external data"),
                      auc.mean$train.ext %>% mutate(metric = "AUC", type = "Train on external data, validate on duplicated original data"),
                      rbs.mean$cv %>% mutate(metric = "rBS", type = "CV on duplicated original data"),
                      rbs.mean$val.ext %>% mutate(metric = "rBS", type = "Train on duplicated original data, validate on external data"),
                      rbs.mean$train.ext %>% mutate(metric = "rBS", type = "Train on external data, validate on duplicated original data"))

ggplot(metrics.mean, aes(pDup, value)) +
  geom_line(aes(color = factor(pFH))) +
  geom_point(aes(color = factor(pFH))) +
  labs(x = "Proportion of duplicates in data", y = "") +
  scale_color_discrete(name = "Proportion of \nduplicates with \nextensive family \nhistory") +
